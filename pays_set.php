...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (empty($code)) {
            ...
            $sql = "INSERT INTO t_pays (nom, code) VALUES (:nom, :code)";
            $stmt = $db->prepare($sql);
            ...
            echo json_encode([
                'status' => 'success',
                'message' => 'Nouveau pays ajouté avec succès',
                'code' => $code
            ]);
        }
        // Sinon, c'est une mise à jour
        else {
            $sql = "UPDATE t_pays SET nom = :nom WHERE code = :code";
            $stmt = $db->prepare($sql);
            $stmt->execute(['nom' => $nom, 'code' => $code]);

            if ($stmt->rowCount() > 0) {
                //echo "pays mis à jour avec succès.";
                // Réponse JSON
                echo json_encode([
                    'status' => 'success',
                ...
                //echo "Aucune modification n'a été effectuée ou la pays n'existe pas.";
                echo json_encode([
                    'status' => 'success',
                    'message' => "Aucune modification n'a été effectuée ou la pays n'existe pas!",
                    'code' => $code
                ]);
            }
        }
...
// Redirection vers la page de liste des payss (à adapter selon votre structure)
// header("Location: liste_payss.php");

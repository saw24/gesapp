...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['code']) && !empty($_POST['code'])) {
        ...
        try {
            ...
            $sql = "DELETE FROM t_filieres WHERE code = :code";
            $stmt = $db->prepare($sql);
            ...
            if ($stmt->execute()) {
                if ($stmt->rowCount() > 0) {
                    ...
                    echo json_encode([
                        'status' => 'success',
                        'message' => 'La filière avec le code ' . $code . ' a été supprimée avec succès.'
                    ]);
                } else {
                    // Aucune ligne affectée, le service n'existait probablement pas
                    echo json_encode([
                        'status' => 'warning',
                        'message' => 'Aucune filière trouvé avec le code ' . $code . '.'
                    ]);
                }
            } else {
                // Erreur lors de l'exécution de la requête
                throw new Exception("Erreur lors de la suppression de la filière.");
            }
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'Le code de la filière est manquant.'
        ]);
    }
} else {

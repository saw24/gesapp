...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nom)) {
        ...
        echo json_encode([
            'status' => 'success',
            'message' => 'Le nom du Niveau est requis.',
            'code' => $code
        ]);
        die();
    }
    ...
    try {
        ...
        if (empty($code)) {
            ...
            $sql = "INSERT INTO t_niveaux_etudiant (nom, code) VALUES (:nom, :code)";
            $stmt = $db->prepare($sql);
        ...
        // Sinon, c'est une mise à jour
            ...
            $sql = "UPDATE t_niveaux_etudiant SET nom = :nom WHERE code = :code";
            $stmt = $db->prepare($sql);
            $stmt->execute(['nom' => $nom, 'code' => $code]);
            ...
            if ($stmt->rowCount() > 0) {
                ...
                echo json_encode([
                    'status' => 'success',
                    'message' => "Aucune modification n'a été effectuée ou le Niveau n'existe pas!",
                    'code' => $code
                ]);
            }
        }

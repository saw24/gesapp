...
<body>
  ...
  <script>
    $(document).ready(function() {
      ...
      function getListe() {
        $.ajax({
          url: 'service_get_liste.php',
          type: 'GET',
          dataType: 'json', // Spécifie que la réponse attendue est du JSON
          success: function(response) {
            if (response.status === 'success') {
              var lignes = ''; // Variable pour stocker les lignes du tableau

              // Parcourir les services retournés et construire les lignes HTML
              $.each(response.donnees, function(index, service) {
                lignes += '<tr>';
                lignes += '<td hidden>' + service.code + '</td>';
                lignes += '<td>' + service.nom + '</td>';
                lignes += '<td>' +

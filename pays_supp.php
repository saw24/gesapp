...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['code']) && !empty($_POST['code'])) {
        ...
        try {
            ...
            $sql = "DELETE FROM t_pays WHERE code = :code";
            $stmt = $db->prepare($sql);
            ...
            if ($stmt->execute()) {
                if ($stmt->rowCount() > 0) {
                    ...
                    echo json_encode([
                        'status' => 'success',
                        'message' => 'Le Pays avec le code ' . $code . ' a été supprimée avec succès.'
                    ]);
                } else {
                ...
                throw new Exception("Erreur lors de la suppression du Pays.");
            }
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'Le code du Pays est manquant.'
        ]);
    }
} else {
